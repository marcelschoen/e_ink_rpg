import '../../assets.dart';
import '../../models/item.dart';

class BlessedBlade extends Sword {
  BlessedBlade() : super() {
    description = 'A holy blade.';
    name = 'Blessed Blade';
    price = 50;
    attackPower = 8.0;
    itemAsset = GameItemAsset.blessedBlade;
  }
}

class Claymore extends LargeSword {
  Claymore() : super() {
    description = 'A two-handed sword.';
    name = 'Claymore';
    price = 35;
    attackPower = 10.0;
    itemAsset = GameItemAsset.claymore;
  }
}

class NobleClaymore extends LargeSword {
  NobleClaymore() : super() {
    description = 'A noble two-handed sword.';
    name = 'Noble Claymore';
    price = 75;
    attackPower = 12.0;
    itemAsset = GameItemAsset.claymore2;
  }
}

class MagicClaymore extends LargeSword {
  MagicClaymore() : super() {
    description = 'A magic two-handed sword.';
    name = 'Magic Claymore';
    price = 180;
    attackPower = 8.0;
    itemAsset = GameItemAsset.claymore3;
  }
}

class BlessedClaymore extends LargeSword {
  BlessedClaymore() : super() {
    description = 'A holy two-handed sword.';
    name = 'Blessed Claymore';
    price = 100;
    attackPower = 15.0;
    itemAsset = GameItemAsset.claymoreBlessed;
  }
}

class Cutlass extends Sword {
  Cutlass() : super() {
    description = 'A regular cutlass.';
    name = 'Regular Cutlass';
    price = 20;
    attackPower = 15.0;
    itemAsset = GameItemAsset.cutlass;
  }
}

class CrystalCutlass extends Sword {
  CrystalCutlass() : super() {
    description = 'A crystal cutlass.';
    name = 'Crystal Cutlass';
    price = 50;
    attackPower = 18.0;
    itemAsset = GameItemAsset.cutlass3;
  }
}

class SteelCutlass extends Sword {
  SteelCutlass() : super() {
    description = 'A steel cutlass.';
    name = 'Steel Cutlass';
    price = 25;
    attackPower = 20.0;
    itemAsset = GameItemAsset.cutlass4;
  }
}

class HardenedCutlass extends Sword {
  HardenedCutlass() : super() {
    description = 'A hardened cutlass.';
    name = 'Hardened Cutlass';
    price = 45;
    attackPower = 16.0;
    itemAsset = GameItemAsset.cutlass5;
  }
}

class IronCutlass extends Sword {
  IronCutlass() : super() {
    description = 'An iron cutlass.';
    name = 'Iron Cutlass';
    price = 45;
    attackPower = 16.0;
    itemAsset = GameItemAsset.cutlass6;
  }
}

class GoldenCutlass extends Sword {
  GoldenCutlass() : super() {
    description = 'A golden cutlass.';
    name = 'Golden Cutlass';
    price = 120;
    attackPower = 10.0;
    itemAsset = GameItemAsset.cutlass7;
  }
}

class AncientCutlass extends Sword {
  AncientCutlass() : super() {
    description = 'An ancient cutlass.';
    name = 'Ancient Cutlass';
    price = 32;
    attackPower = 25.0;
    itemAsset = GameItemAsset.cutlass8;
  }
}

class OfficerCutlass extends Sword {
  OfficerCutlass() : super() {
    description = 'An officers cutlass.';
    name = 'Officers Cutlass';
    price = 85;
    attackPower = 20.0;
    itemAsset = GameItemAsset.cutlass9;
  }
}

class ValuableDagger extends Dagger {
  ValuableDagger() : super() {
    description = 'A valuable dagger.';
    name = 'Valuable Dagger';
    price = 68;
    attackPower = 8.0;
    itemAsset = GameItemAsset.dagger3;
  }
}

class IronDagger extends Dagger {
  IronDagger() : super() {
    description = 'An iron dagger.';
    name = 'Iron Dagger';
    price = 14;
    attackPower = 10.0;
    itemAsset = GameItemAsset.dagger6;
  }
}

class RustyDagger extends Dagger {
  RustyDagger() : super() {
    description = 'A rusty dagger.';
    name = 'Rusty Dagger';
    price = 10;
    attackPower = 6.0;
    itemAsset = GameItemAsset.dagger7;
  }
}

class NewDagger extends Dagger {
  NewDagger() : super() {
    description = 'A new dagger.';
    name = 'New Dagger';
    price = 15;
    attackPower = 8.0;
    itemAsset = GameItemAsset.dagger;
  }
}

class OldDagger extends Dagger {
  OldDagger() : super() {
    description = 'An old dagger.';
    name = 'Old Dagger';
    price = 7;
    attackPower = 8.0;
    itemAsset = GameItemAsset.daggerOld;
  }
}

class DemonBlade extends Sword {
  DemonBlade() : super() {
    description = 'A demonish blade. It radiates evil power!';
    name = 'Demon Blade';
    price = 166;
    attackPower = 35.0;
    itemAsset = GameItemAsset.demonBlade;
  }
}

class DoubleSword extends LargeSword {
  DoubleSword() : super() {
    description = 'A double sword.';
    name = 'Double Sword';
    price = 24;
    attackPower = 14.0;
    itemAsset = GameItemAsset.doubleSword;
  }
}

class DoubleSwordOld extends LargeSword {
  DoubleSwordOld() : super() {
    description = 'An old double sword.';
    name = 'Old Double Sword';
    price = 13;
    attackPower = 13.0;
    itemAsset = GameItemAsset.doubleSwordOld;
  }
}

class NobleDoubleSword extends LargeSword {
  NobleDoubleSword() : super() {
    description = 'A noble double sword.';
    name = 'Noble Double Sword';
    price = 57;
    attackPower = 22.0;
    itemAsset = GameItemAsset.doubleSword2;
  }
}

class HolyDoubleSword extends LargeSword {
  HolyDoubleSword() : super() {
    description = 'A holy double sword.';
    name = 'Holy Double Sword';
    price = 190;
    attackPower = 34.0;
    itemAsset = GameItemAsset.doubleSword3;
  }
}

class ElvenBroadSword extends LargeSword {
  ElvenBroadSword() : super() {
    description = 'An elven broadsword.';
    name = 'Elven Broadsword';
    price = 28;
    attackPower = 22.0;
    itemAsset = GameItemAsset.elvenBroadsword;
  }
}

class ElvenDagger extends Dagger {
  ElvenDagger() : super() {
    description = 'An elven dagger.';
    name = 'Elven Dagger';
    price = 18;
    attackPower = 12.0;
    itemAsset = GameItemAsset.elvenDagger;
  }
}

class ElvenShortSword extends Sword {
  ElvenShortSword() : super() {
    description = 'An elven shortsword.';
    name = 'Elven Shortsword';
    price = 34;
    attackPower = 18.0;
    itemAsset = GameItemAsset.elvenShortsword;
  }
}



class Falchion extends Sword {
  Falchion() : super() {
    description = 'A new falchion.';
    name = 'Falchion';
    price = 24;
    attackPower = 12.0;
    itemAsset = GameItemAsset.falchion;
  }
}

class PirateFalchion extends Sword {
  PirateFalchion() : super() {
    description = 'A pirate\'s falchion.';
    name = 'Pirate Falchion';
    price = 19;
    attackPower = 13.0;
    itemAsset = GameItemAsset.falchion2;
  }
}

class GoldenFalchion extends Sword {
  GoldenFalchion() : super() {
    description = 'A golden falchion.';
    name = 'Golden Falchion';
    price = 112;
    attackPower = 10.0;
    itemAsset = GameItemAsset.falchion2Old;
  }
}

class OrcishFalchion extends Sword {
  OrcishFalchion() : super() {
    description = 'An orcish falchion.';
    name = 'Orcish Falchion';
    price = 33;
    attackPower = 17.0;
    itemAsset = GameItemAsset.falchion3;
  }
}

class HeavyFalchion extends Sword {
  HeavyFalchion() : super() {
    description = 'A heavy falchion.';
    name = 'Heavy Falchion';
    price = 33;
    attackPower = 16.0;
    itemAsset = GameItemAsset.falchion4;
  }
}

class CleavingFalchion extends Sword {
  CleavingFalchion() : super() {
    description = 'A cleaving falchion.';
    name = 'Cleaving Falchion';
    price = 42;
    attackPower = 19.0;
    itemAsset = GameItemAsset.falchion5;
  }
}

class SteelFalchion extends Sword {
  SteelFalchion() : super() {
    description = 'A steel falchion.';
    name = 'Steel Falchion';
    price = 14;
    attackPower = 15.0;
    itemAsset = GameItemAsset.falchion6;
  }
}

class NobleFalchion extends Sword {
  NobleFalchion() : super() {
    description = 'A noble falchion.';
    name = 'Noble Falchion';
    price = 54;
    attackPower = 15.0;
    itemAsset = GameItemAsset.falchion7;
  }
}

class GoldenSword extends LargeSword {
  GoldenSword() : super() {
    description = 'A golden broadsword.';
    name = 'Golden Broadsword';
    price = 140;
    attackPower = 17.0;
    itemAsset = GameItemAsset.goldenSword;
  }
}

class GreatSword extends LargeSword {
  GreatSword() : super() {
    description = 'A greatsword.';
    name = 'Greatsword';
    price = 37;
    attackPower = 24.0;
    itemAsset = GameItemAsset.greatSword;
  }
}

class OldGreatSword extends LargeSword {
  OldGreatSword() : super() {
    description = 'An old greatsword.';
    name = 'Old Greatsword';
    price = 27;
    attackPower = 20.0;
    itemAsset = GameItemAsset.greatSwordOld;
  }
}

class RunicGreatSword extends LargeSword {
  RunicGreatSword() : super() {
    description = 'A greatsword with mysterious runes.';
    name = 'Runic Greatsword';
    price = 177;
    attackPower = 28.0;
    itemAsset = GameItemAsset.greatSword2;
  }
}

class NobleGreatSword extends LargeSword {
  NobleGreatSword() : super() {
    description = 'A noble man\'s greatsword.';
    name = 'Noble Greatsword';
    price = 126;
    attackPower = 25.0;
    itemAsset = GameItemAsset.greatSword3;
  }
}

class SteelGreatSword extends LargeSword {
  SteelGreatSword() : super() {
    description = 'A steel greatsword.';
    name = 'Steel Greatsword';
    price = 56;
    attackPower = 23.0;
    itemAsset = GameItemAsset.greatSword3Old;
  }
}

class MagicGreatSword extends LargeSword {
  MagicGreatSword() : super() {
    description = 'A magical greatsword.';
    name = 'Magic Greatsword';
    price = 192;
    attackPower = 28.0;
    itemAsset = GameItemAsset.greatSword4;
  }
}

class SteelKatana extends LargeSword {
  SteelKatana() : super() {
    description = 'A steel katana.';
    name = 'Steel Katana';
    price = 53;
    attackPower = 28.0;
    itemAsset = GameItemAsset.katana;
  }
}

class Katana extends LargeSword {
  Katana() : super() {
    description = 'An elegant katana.';
    name = 'Katana';
    price = 48;
    attackPower = 25.0;
    itemAsset = GameItemAsset.katana1;
  }
}

class MasterKatana extends LargeSword {
  MasterKatana() : super() {
    description = 'An master katana.';
    name = 'Master Katana';
    price = 82;
    attackPower = 30.0;
    itemAsset = GameItemAsset.katana2;
  }
}

class Knife extends Dagger {
  Knife() : super() {
    description = 'A small knife.';
    name = 'Knife';
    price = 17;
    attackPower = 16.0;
    itemAsset = GameItemAsset.knife;
  }
}

class OldLongSword extends LargeSword {
  OldLongSword() : super() {
    description = 'An old longsword.';
    name = 'Old Longsword';
    price = 22;
    attackPower = 26.0;
    itemAsset = GameItemAsset.longSword;
  }
}

class LongSword extends LargeSword {
  LongSword() : super() {
    description = 'A longsword.';
    name = 'Longsword';
    price = 26;
    attackPower = 29.0;
    itemAsset = GameItemAsset.longSwordOld;
  }
}

class NobleLongSword extends LargeSword {
  NobleLongSword() : super() {
    description = 'A noble longsword.';
    name = 'Noble Longsword';
    price = 50;
    attackPower = 31.0;
    itemAsset = GameItemAsset.longSword2;
  }
}

class WeirdLongSword extends LargeSword {
  WeirdLongSword() : super() {
    description = 'A mysterious, weird longsword.';
    name = 'Weird Longsword';
    price = 0;
    attackPower = 25.0;
    itemAsset = GameItemAsset.longSword3;
  }
}

class MagicLongSword extends LargeSword {
  MagicLongSword() : super() {
    description = 'A magical longsword.';
    name = 'Magical Longsword';
    price = 110;
    attackPower = 26.0;
    itemAsset = GameItemAsset.longSword5;
  }
}

class SturdyLongSword extends LargeSword {
  SturdyLongSword() : super() {
    description = 'A sturdy longsword.';
    name = 'Sturdy Longsword';
    price = 40;
    attackPower = 29.0;
    itemAsset = GameItemAsset.longSword6;
  }
}

class EnchantedLongSword extends LargeSword {
  EnchantedLongSword() : super() {
    description = 'An enchanted longsword.';
    name = 'Enchanted Longsword';
    price = 190;
    attackPower = 40.0;
    itemAsset = GameItemAsset.longSword7;
  }
}

class OrcishDagger extends Dagger {
  OrcishDagger() : super() {
    description = 'An orcish dagger.';
    name = 'Orcish Dagger';
    price = 17;
    attackPower = 15.0;
    itemAsset = GameItemAsset.orcishDagger;
  }
}

class OrcishGreatsword extends LargeSword {
  OrcishGreatsword() : super() {
    description = 'An orcish greatsword.';
    name = 'Orcish Greatsword';
    price = 29;
    attackPower = 35.0;
    itemAsset = GameItemAsset.orcishGreatSword;
  }
}

class OrcishLongsword extends LargeSword {
  OrcishLongsword() : super() {
    description = 'An orcish longsword.';
    name = 'Orcish Longsword';
    price = 36;
    attackPower = 38.0;
    itemAsset = GameItemAsset.orcishLongSword;
  }
}

class OrcishShortsword extends Sword {
  OrcishShortsword() : super() {
    description = 'An orcish shortsword.';
    name = 'Orcish Shortsword';
    price = 26;
    attackPower = 24.0;
    itemAsset = GameItemAsset.orcishShortSword;
  }
}

class Quickblade extends Sword {
  Quickblade() : super() {
    description = 'A superfast blade!';
    name = 'Quickblade';
    price = 126;
    attackPower = 40.0;
    itemAsset = GameItemAsset.quickblade;
  }
}

class Rapier extends Sword {
  Rapier() : super() {
    description = 'A rapier!';
    name = 'Rapier';
    price = 26;
    attackPower = 28.0;
    itemAsset = GameItemAsset.rapier;
  }
}

class MagicRapier extends Sword {
  MagicRapier() : super() {
    description = 'A magic rapier!';
    name = 'Magic Rapier';
    price = 86;
    attackPower = 18.0;
    itemAsset = GameItemAsset.rapier2;
  }
}

class NobleRapier extends Sword {
  NobleRapier() : super() {
    description = 'A noble rapier!';
    name = 'Noble Rapier';
    price = 46;
    attackPower = 38.0;
    itemAsset = GameItemAsset.rapier3;
  }
}

class SilverSabre extends Sword {
  SilverSabre() : super() {
    description = 'A silver sabre useful against the undead!';
    name = 'Silver Sabre';
    price = 60;
    attackPower = 35.0;
    itemAsset = GameItemAsset.sabreSilver;
  }
}

class BloodSabre extends Sword {
  BloodSabre() : super() {
    description = 'A blood-thirsty sabre!';
    name = 'Blood Sabre';
    price = 44;
    attackPower = 46.0;
    itemAsset = GameItemAsset.sabre2;
  }
}

class Scimitar extends Sword {
  Scimitar() : super() {
    description = 'A shiny scimitar!';
    name = 'Scimitar';
    price = 27;
    attackPower = 20.0;
    itemAsset = GameItemAsset.scimitar;
  }
}

class OldScimitar extends Sword {
  OldScimitar() : super() {
    description = 'An old scimitar!';
    name = 'Old Scimitar';
    price = 17;
    attackPower = 14.0;
    itemAsset = GameItemAsset.scimitarOld;
  }
}

class GoldenScimitar extends Sword {
  GoldenScimitar() : super() {
    description = 'A gold-adorned scimitar!';
    name = 'Golden Scimitar';
    price = 76;
    attackPower = 44.0;
    itemAsset = GameItemAsset.scimitar2;
  }
}

class MagicalScimitar extends Sword {
  MagicalScimitar() : super() {
    description = 'A magical scimitar!';
    name = 'Magical Scimitar';
    price = 114;
    attackPower = 40.0;
    itemAsset = GameItemAsset.scimitar3;
  }
}

class RustyShortSword extends Sword {
  RustyShortSword() : super() {
    description = 'A bit rusty and pretty weak, but it can still do some damage.';
    name = 'Rusty short sword';
    price = 14;
    attackPower = 5.0;
    itemAsset = GameItemAsset.shortSword;
  }
}

class OldShortSword extends Sword {
  OldShortSword() : super() {
    description = 'An old, worn short sword.';
    name = 'Old Short Sword';
    price = 20;
    attackPower = 6.0;
    itemAsset = GameItemAsset.shortSwordOld;
  }
}

class MagicalShortSword extends Sword {
  MagicalShortSword() : super() {
    description = 'A magical short sword.';
    name = 'Magical Short Sword';
    price = 90;
    attackPower = 55.0;
    itemAsset = GameItemAsset.shortSword2;
  }
}

class SturdyShortSword extends Sword {
  SturdyShortSword() : super() {
    description = 'A sturdy short sword.';
    name = 'Sturdy Short Sword';
    price = 28;
    attackPower = 26.0;
    itemAsset = GameItemAsset.shortSword2Old;
  }
}

class NobleShortSword extends Sword {
  NobleShortSword() : super() {
    description = 'A nobleman short sword.';
    name = 'Noble Short Sword';
    price = 78;
    attackPower = 30.0;
    itemAsset = GameItemAsset.shortSword3;
  }
}

class GoldenShortSword extends Sword {
  GoldenShortSword() : super() {
    description = 'A gold-adorned short sword.';
    name = 'Gold Short Sword';
    price = 120;
    attackPower = 20.0;
    itemAsset = GameItemAsset.shortSword5;
  }
}

class SteelShortSword extends Sword {
  SteelShortSword() : super() {
    description = 'A steel short sword.';
    name = 'Steel Short Sword';
    price = 45;
    attackPower = 30.0;
    itemAsset = GameItemAsset.shortSword6;
  }
}

class DarkShortSword extends Sword {
  DarkShortSword() : super() {
    description = 'A short sword that exhumes dark power!';
    name = 'Dark Short Sword';
    price = 200;
    attackPower = 60.0;
    itemAsset = GameItemAsset.shortSword7;
  }
}

class TripleSword extends LargeSword {
  TripleSword() : super() {
    description = 'A massive triple sword!';
    name = 'Triple Sword';
    price = 90;
    attackPower = 50.0;
    itemAsset = GameItemAsset.tripleSword;
  }
}

class OldTripleSword extends LargeSword {
  OldTripleSword() : super() {
    description = 'A big old triple sword!';
    name = 'Old Triple Sword';
    price = 70;
    attackPower = 40.0;
    itemAsset = GameItemAsset.tripleSwordOld;
  }
}

class NobleTripleSword extends LargeSword {
  NobleTripleSword() : super() {
    description = 'A nobleman triple sword!';
    name = 'Noble Triple Sword';
    price = 140;
    attackPower = 56.0;
    itemAsset = GameItemAsset.tripleSword2;
  }
}

class DragonTripleSword extends LargeSword {
  DragonTripleSword() : super() {
    description = 'A dragon-scale adorned triple sword!';
    name = 'Dragon Triple Sword';
    price = 180;
    attackPower = 76.0;
    itemAsset = GameItemAsset.tripleSword3;
  }
}

class Tsurugi extends Sword {
  Tsurugi() : super() {
    description = 'An elegant, fast weapon!';
    name = 'Tsurugi';
    price = 140;
    attackPower = 70.0;
    itemAsset = GameItemAsset.tsurugi;
  }
}

class TwoHandedSword extends LargeSword {
  TwoHandedSword() : super() {
    description = 'A heavy two-handed sword!';
    name = 'Two-Handed Sword';
    price = 58;
    attackPower = 60.0;
    itemAsset = GameItemAsset.twoHandedSword;
  }
}
