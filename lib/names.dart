...
class NewNameGenerator {
  ...
  String compose(int syllables) {
    if (syllables > 2 && mid.isEmpty) {
      throw NameGeneratorException("You are trying to create a name with more than 3 parts, which requires middle parts, which you have none in the file $syllablesFile. You should add some. Every word, which doesn't have + or - for a prefix is counted as a middle part.");
    }
    if (pre.isEmpty) {
      throw NameGeneratorException("You have no prefixes to start creating a name. add some and use \"-\" prefix, to identify it as a prefix for a name. (example: -asd)");
    }
    if (sur.isEmpty) {
      throw NameGeneratorException("You have no suffixes to end a name. add some and use \"+\" prefix, to identify it as a suffix for a name. (example: +asd)");
    }
    if (syllables < 1) throw NameGeneratorException("compose(int syls) can't have less than 1 syllable");
    ...
    if (syllables > 2) {
      if (expectsVocal(pre[a])) {
        ...
        if (containsVocFirst(mid) == false) {
          throw NameGeneratorException("Expecting \"middle\" part starting with vocal, " "but there is none. You should add one, or remove requirement for one.. ");
        }
      }
      if (expectsConsonant(pre[a])) {
        expecting = 2;
        if (containsConsFirst(mid) == false) {
          throw NameGeneratorException("Expecting \"middle\" part starting with consonant, " "but there is none. You should add one, or remove requirement for one.. ");
        }
      }
    } else {
      if (expectsVocal(pre[a])) {
        ...
        if (containsVocFirst(sur) == false) {
          throw NameGeneratorException("Expecting \"suffix\" part starting with vocal, " "but there is none. You should add one, or remove requirement for one.. ");
        }
      }
      if (expectsConsonant(pre[a])) {
        expecting = 2;
        if (containsConsFirst(sur) == false) {
          throw NameGeneratorException("Expecting \"suffix\" part starting with consonant, " "but there is none. You should add one, or remove requirement for one.. ");
        }
      }
    }
    if (vocalLast(pureSyl(pre[a])) && allowVocs(mid) == false) {
      throw NameGeneratorException("Expecting \"middle\" part that allows last character of prefix to be a vocal, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the prefix used, was : \"${pre[a]}\", whichmeans there should be a part available, that has \"-v\" requirement or no requirements for previous syllables at all.");
    }

    if (consonantLast(pureSyl(pre[a])) && allowCons(mid) == false) {
      throw NameGeneratorException("Expecting \"middle\" part that allows last character of prefix to be a consonant, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the prefix used, was : \"${pre[a]}\", whichmeans there should be a part available, that has \"-c\" requirement or no requirements for previous syllables at all.");
    }
    ...
    for (int i = 0; i < b.length - 2; i++) {
      ...
      if (vocalLast(pureSyl(mid[b[i]])) && allowVocs(mid) == false && syllables > 3) {
        throw NameGeneratorException("Expecting \"middle\" part that allows last character of last syllable to be a vocal, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the part used, was : \"${mid[b[i]]}\", which means there should be a part available, that has \"-v\" requirement or no requirements for previous syllables at all.");
      }

      if (consonantLast(pureSyl(mid[b[i]])) && allowCons(mid) == false && syllables > 3) {
        throw NameGeneratorException("Expecting \"middle\" part that allows last character of last syllable to be a consonant, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the part used, was : \"${mid[b[i]]}\", which means there should be a part available, that has \"-c\" requirement or no requirements for previous syllables at all.");
      }
      if (i == b.length - 3) {
        if (vocalLast(pureSyl(mid[b[i]])) && allowVocs(sur) == false) {
          throw NameGeneratorException("Expecting \"suffix\" part that allows last character of last syllable to be a vocal, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the part used, was : \"${mid[b[i]]}\", which means there should be a suffix available, that has \"-v\" requirement or no requirements for previous syllables at all.");
        }

        if (consonantLast(pureSyl(mid[b[i]])) && allowCons(sur) == false) {
          throw NameGeneratorException("Expecting \"suffix\" part that allows last character of last syllable to be a consonant, but there is none. You should add one, or remove requirements that cannot be fulfilled.. the part used, was : \"${mid[b[i]]}\", which means there should be a suffix available, that has \"-c\" requirement or no requirements for previous syllables at all.");
        }
      }
